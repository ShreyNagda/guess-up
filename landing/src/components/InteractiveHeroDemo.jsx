...
export const InteractiveHeroDemo = () => {
  ...
  // Keyboard Shortcuts (ArrowDown / Space = Correct, ArrowUp / Esc = Pass)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleOrientation = (event) => {
      ...
      const gamma = event.gamma; // [-90, 90] roll
      const beta = event.beta;   // [-180, 180] pitch

      // In landscape or forehead mode:
      // Tilt down (Correct) -> beta > 45 || gamma > 45
      // Tilt up (Pass) -> beta < -45 || gamma < -45
      if (beta > 45 || gamma > 45) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col gap-4">
      ...
      <div className="relative w-full aspect-video min-h-[360px] sm:min-h-[420px] bg-[#0E0C1C] border-4 border-[#1E1938] rounded-[32px] p-4 sm:p-6 flex flex-col justify-between overflow-hidden text-white font-sans shadow-2xl">
          ...
          {flashColor && (
            <motion.div
              ...
              className={`absolute inset-0 z-30 pointer-events-none rounded-[28px] ${
                flashColor === "green" ? "bg-emerald-500/30 border-4 border-emerald-500" : "bg-red-500/30 border-4 border-red-500"
              }`}
            />
          )}
        </AnimatePresence>
        ...
        {/* Header HUD Bar */}
        <div className="flex items-center justify-between z-20 bg-surface-card/60 backdrop-blur-md px-4 py-2 rounded-2xl border border-border/40 text-xs font-black">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-primary animate-ping" />
            <span className="uppercase tracking-widest text-primary">Live Playable Demo</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="bg-primary/15 text-primary border border-primary/30 px-2.5 py-0.5 rounded-full text-[0.65rem] font-mono">
              20 Indian Cards Loaded
            </span>
          </div>
        </div>
        ...
        {gameState === "idle" && (
          ...
          >
            <div className="w-14 h-14 rounded-2xl bg-primary/15 border-2 border-primary flex items-center justify-center text-primary font-black shadow-lg">
            ...
            <div>
              ...
              <p className="text-muted text-xs sm:text-sm mt-1 leading-relaxed">
                Test tilt sensing on mobile or keyboard controls on desktop. Preloaded with 20 iconic Indian pop-culture cards!
              </p>
            </div>
            ...
            <div className="flex flex-col gap-1.5 w-full">
              <span className="text-[0.65rem] font-black uppercase text-muted tracking-wider flex items-center justify-center gap-1">
                <Clock className="w-3 h-3 text-primary" /> Select Round Duration:
              </span>
            ...
            >
              <Play className="w-4 h-4 fill-accent" /> Start {selectedTimer}s Round Now
            </button>
            ...
            {sensorPermission === "prompt" && (
              ...
              >
                <Smartphone className="w-3 h-3" /> Enable Mobile Tilt Sensing Permissions
              </button>
            )}
          </motion.div>
        )}
        ...
        {gameState === "playing" && (
          <div className="flex flex-col justify-between h-full z-20 py-2">
            {/* Top Score Bar */}
            <div className="flex items-center justify-between bg-surface-card/80 border border-border/40 px-4 py-2 rounded-xl text-xs font-black">
              <div className="flex items-center gap-2">
                <Clock className="w-4 h-4 text-primary animate-pulse" />
                <span className="text-lg font-mono font-black text-primary">{timeLeft}s</span>
              </div>

              {gameMode === "team" && (
                <div className="flex items-center gap-3">
                  <span className={`px-2.5 py-0.5 rounded-full text-[0.65rem] ${activeTeam === "Team A" ? "bg-cyan-500/20 text-cyan-400 border border-cyan-500/30" : "bg-pink-500/20 text-pink-400 border border-pink-500/30"}`}>
                    Current: {activeTeam}
                  </span>
                </div>
              )}
              ...
              <div className="flex items-center gap-3">
                <span className="text-emerald-400 font-black">✓ {score} Correct</span>
                <span className="text-red-400 font-black">✗ {passedCount} Pass</span>
              </div>
            </div>
            ...
            <motion.div
              ...
              className="my-auto bg-gradient-to-b from-[#1E1938] to-[#0E0C1C] border-2 border-primary/60 rounded-3xl p-6 sm:p-8 text-center shadow-2xl flex flex-col justify-center items-center gap-2 min-h-[160px]"
            >
        ...
        {gameState === "ended" && (
          ...
          >
            ...
            <div>
              <h3 className="text-2xl font-black text-primary uppercase tracking-tight">Round Complete!</h3>
              <p className="text-xs text-muted mt-1">
                You scored <strong>{score} correct points</strong> in {selectedTimer} seconds!
              </p>
            </div>
            ...
            {gameMode === "team" && (
              <div className="w-full grid grid-cols-2 gap-3 p-3 rounded-2xl bg-surface-card border border-border">
                <div className="flex flex-col">
                  <span className="text-[0.65rem] font-bold text-cyan-400 uppercase">Team A Score</span>
                  <span className="text-xl font-black text-white">{teamScores["Team A"]}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[0.65rem] font-bold text-pink-400 uppercase">Team B Score</span>
                  <span className="text-xl font-black text-white">{teamScores["Team B"]}</span>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-3 w-full">
              {gameMode === "team" && (
                <button
                  onClick={() => {
                    setActiveTeam(activeTeam === "Team A" ? "Team B" : "Team A");
                    startGame();
                  }}
              ...
              <button
                onClick={startGame}
                className="flex-1 py-3 bg-white/10 text-white font-extrabold text-xs uppercase tracking-wider rounded-2xl hover:bg-white/20 transition-all cursor-pointer flex items-center justify-center gap-1.5"
              >
